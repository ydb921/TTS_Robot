 ...
 * delay_ms(unsigned long num_ms)    //replaced by HAL_Delay()
...
static int accel_self_test(long *bias_regular, long *bias_st)
{
    ...
    for(jj = 0; jj < 3; jj++) {
        ...
        if (st_shift[jj]) 
        {
          ...
          {
            printf("st_shift_var:%f > %f\n",st_shift_var,test.max_accel_var);
            result |= 1 << jj;
          }
         } 
        else if ((st_shift_cust < test.min_g) || (st_shift_cust > test.max_g))
        {
          printf("st_shift_cust:%f (%f,%f)",st_shift_cust,test.min_g,test.max_g);
            result |= 1 << jj;
        }
    }
...
int mpu_run_self_test(long *gyro, long *accel)
{
    ...
    printf("accel_self_test_result= %d\n",accel_result );
    gyro_result = gyro_self_test(gyro, gyro_st);
    printf("gyro_self_test_result= %d\n",gyro_result );
